...
/**************************************************************************************************
* Sage Pay Direct PHP Kit Welcome Page
***************************************************************************************************
...
<html>
<head>
	<title>Direct PHP Welcome Page</title>
...
<body>
    <div id="pageContainer">
        ...
        <div id="content">
            <div id="contentHeader">Welcome to the Sage Pay Direct PHP Kit </div>
				<p>If you are viewing this page in your browser at <strong><? echo strtolower(substr($_SERVER['SERVER_PROTOCOL'],0,strpos($_SERVER['SERVER_PROTOCOL'],"/"))) . "://" .  $_SERVER["SERVER_NAME"] . $_SERVER["PHP_SELF"]; ?></strong> then you have correctly set up your virtual directory.</p>
				<?
					...
					if ($strVendorName=="[your Sage Pay Vendor Name]" or $strYourSiteFQDN=="http://[your web site]/" or $strDatabasePassword=="[your database user password]")
					{
						...
						echo "<tr><td class=\"fieldLabel\" width=\"40%\">Sage Pay Vendor Name:</td><td>" . $strVendorName . "</td></tr>";
				 ...
				 ?> 
				<tr>
					<td class="fieldLabel">Products in Database:</td><td><? echo $iTotal ?></td>
				</tr>
			</table>
				<p>
					<? if ($strConnectTo=="LIVE") {
						echo "<span class=\"warning\">Your kit is pointing at the Live Sage Pay environment.  You should only do this once your have completed testing on both the Sage Pay Simulator AND Test servers, have sent your GoLive request to the technical support team and had confirmation that your account has been set up. <br><br>";
					...
					elseif ($strConnectTo=="TEST")
						echo "Your kit is pointing at the Sage Pay TEST environment.  This is an exact replica of the Live systems except that no banks are attached, so no authorisation requests are sent, nothing is settled and you can use our test card numbers when making payments. You should only use the test environment after you have completed testing using the Sage Pay Simulator AND the Sage Pay support team have mailed you to let you know your account has been created.<br><br><span class=\"warning\"><strong>If you are already set up on Live and are testing additional functionality, DO NOT leave your kit set to Test or you will not receive any money for your transactions!</strong></span>";
					else
						echo "Your kit is currently pointing at the Simulator. This is an Expert System provided by Sage Pay to enable you to build and configure your site correctly, to debug the messages you send to Sage Pay Direct and practise handling responses from it.  No customers are charged, no money is moved around.  The Simulator is for development and testing ONLY.";
					?>
				</p>
